...
>
> 1. **`docs/CHANGES.md`** — **LEER PRIMERO**: mapa de ruta TPI, todos los changes organizados por criterio de la rúbrica de 200 pts.
...
>
> **Este change es el CORAZÓN de la migración al TPI. Cubre infraestructura (UoW, core, rate limiting, /api/v1), modelo de datos (ERD v5), servicios (FSM, soft delete), frontend (Zustand, TanStack Query) y la refactorización completa del seed + entidades faltantes del spec.**
...
- [x] 4.7 Refactorizar `backend/app/services/recommendation_service.py`: misma refactorización — 🔵 **Archivo eliminado (feature Extra removida, ver docs/CHANGES.md)**
...
- [x] 11.9 Refactorizar `frontend/src/hooks/useRecommendations.ts`: usar `useQuery` — 🔵 **Archivo eliminado (feature Extra removida, ver docs/CHANGES.md)**
...
## Fase 15: Seed de Roles, Admin User y DireccionEntrega (TPI-2)

> **Qué recupera**: Puntos perdidos por ausencia de `roles`, `usuario_rol`, `direcciones_entrega` en el modelo de datos, falta de seed con 4 roles + admin user, 3 formas de pago (EFECTIVO, TRANSFERENCIA, MERCADOPAGO) y 6 estados de pedido (RN-AU01, RN-DA01–DA08).
...
## Fase 16: Alinear FSM a 6 Estados del Spec (TPI-2)
...
## Fase 17: Migrar Auth Routes a UoW (TPI-1)
...
## Fase 18: CORS Fix — Configurable por Entorno (TPI-1)
