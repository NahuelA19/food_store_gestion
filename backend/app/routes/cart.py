...
async def get_cart_by_id(
    ...
    cart = await get_cart_with_items(cart_id, current_user.id, db)
    return await get_cart_response(cart)
...
async def add_item(
    ...
    updated_cart = await add_item_to_cart(cart_id, body, db)
    logger.info(
...
async def update_item_quantity(
    ...
    # Verify cart belongs to user
    await get_cart_with_items(cart_id, current_user.id, db)

    updated_cart = await update_cart_item_quantity(cart_id, item_id, body, db)
    logger.info(
...
async def remove_item(
    ...
    updated_cart = await remove_cart_item(cart_id, item_id, db)
    logger.info(
...
async def clear_items(
    ...
    updated_cart = await clear_cart_items(cart_id, db)
