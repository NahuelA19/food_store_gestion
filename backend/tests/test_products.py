...
def test_list_products_empty(test_client: TestClient):
    ...
    response = test_client.get("/api/products/?page=1&limit=20")
...
def test_list_products_filter_by_category(test_client: TestClient):
    ...
    response = test_client.get(f"/api/products/?category_id={cat1_id}")
    assert response.status_code == 200
    data = response.json()
...
def test_list_products_filter_by_price(test_client: TestClient):
    ...
    response = test_client.get("/api/products/?min_price=2.5&max_price=4.0")
    assert response.status_code == 200
    data = response.json()
    assert all(
...
def test_list_products_search(test_client: TestClient):
    ...
    # Search
    response = test_client.get("/api/products/?search=Spaghetti")
    assert response.status_code == 200
    data = response.json()
...
def test_toggle_product_availability(test_client: TestClient):
    ...
    # Toggle availability
    response = test_client.put(f"/api/products/{product_id}/availability")
    assert response.status_code == 200
    data = response.json()
...
def test_get_related_products(test_client: TestClient):
    ...
    response = test_client.get(f"/api/products/{product_id}/related")
    assert response.status_code == 200
    data = response.json()
