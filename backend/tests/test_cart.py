...
class TestCartEndpoints:
    ...
    async def test_get_cart_creates_on_demand(
        ...
        response = await async_client.get("/api/v1/carts/", headers=auth_headers)
    ...
    async def test_add_item_to_cart(
        ...
        # Get cart
        cart_resp = await async_client.get("/api/v1/carts/", headers=auth_headers)
    ...
    async def test_user_cart_isolation(
        ...
        cart1 = await async_client.get("/api/v1/carts/", headers=headers1)
        cart1_id = cart1.json()["id"]

        await async_client.post(
            f"/api/v1/carts/{cart1_id}/items",
            json={"product_id": prod_id, "quantity": 3},
            headers=headers1,
        )
        ...
        cart2 = await async_client.get("/api/v1/carts/", headers=headers2)
